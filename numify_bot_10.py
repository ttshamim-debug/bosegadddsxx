...
TRAFFIC_CHANNEL_ID = "@numifyotp"          # hidden — used only for join verification, never shown to users
...
SUPPORT_GROUP_LINK = "https://t.me/numifysupport"
SUPPORT_GROUP_ID   = "@numifysupport"      # hidden — used only for join verification, never shown to users
CURRENCY           = "৳"
MIN_WITHDRAW       = 1
MINIMUM_WITHDRAWAL = MIN_WITHDRAW
DEFAULT_OTP_RATE   = 0.20                  # ৳ per OTP
SPECIAL_RATE_BONUS = 0.70                  # ৳ extra per OTP for special-rate users
...
# ╔══════════════════════════════════════════════════════════╗
# ║  Group guard — bot only talks in private chats.           ║
# ║  Registered FIRST so it wins for every group/channel      ║
# ║  message except the two chats we deliberately monitor     ║
# ║  (OTP forward group, range monitor channel). This stops   ║
# ║  ordinary group chatter (e.g. in NUMIFY SUPPORT) from      ║
# ║  ever triggering /start, the fallback menu, or any reply  ║
# ║  keyboard inside the group.                                ║
# ╚══════════════════════════════════════════════════════════╝
_MONITORED_GROUP_CHATS = {OTP_CHAT_ID, RANGE_CHANNEL_ID}

@bot.message_handler(
    func=lambda m: m.chat.type != "private" and m.chat.id not in _MONITORED_GROUP_CHATS,
    content_types=["text", "photo", "sticker", "document", "video", "voice", "animation"],
)
def _ignore_non_private_chats(m):
    return  # silently ignore — no reply, no keyboard, nothing posted to the group
...
def force_join_kb() -> types.InlineKeyboardMarkup:
    ...
    kb.add(types.InlineKeyboardButton("👥 NUMIFY SUPPORT", url=SUPPORT_GROUP_LINK))
    kb.add(types.InlineKeyboardButton("✅ Verify", callback_data="verify_join"))
    return kb


# Chats a user must be a member of before using the bot.
# (label used only in log messages — never shown to the user)
_REQUIRED_CHATS = [
    ("OTP Group", OTP_CHAT_ID),
    ("NUMIFY CHANEL", TRAFFIC_CHANNEL_ID),
    ("NUMIFY SUPPORT", SUPPORT_GROUP_ID),
]


def has_joined_required_channels(uid: int) -> bool:
    allowed = {"creator", "administrator", "member", "restricted"}
    for label, chat_id in _REQUIRED_CHATS:
        try:
            member = bot.get_chat_member(chat_id, uid)
            if member.status not in allowed:
                return False
        except Exception as e:
            log.warning(f"force join check failed for {uid} on {label}: {e}")
            return False
    return True
...
def cmd_start(m):
                                ...
                                if ref_user["ref_count"] >= target and not ref_user["vip"]:
                                    try:
                                        bot.send_message(
                                            ...
                                            f"📩 Contact admin @{md_escape(ADMIN_USERNAME)} to claim your *VIP* ⭐"
                                        )
                                    except Exception: pass
    ...
    if new_user:
        text = (
            ...
            "🤝 To unlock the bot, please join all <b>3</b> of our channels below,\n"
...
def cb_verify_join(cq):
    if not has_joined_required_channels(cq.from_user.id):
        bot.answer_callback_query(
            cq.id,
            "❗ You must join all 3 groups/channels first, then tap Verify again.",
            show_alert=True,
        )
        return
    try:
...
def h_number(m):
    ...
    if not is_admin(m.from_user.id) and not has_joined_required_channels(m.from_user.id):
        bot.send_message(
            m.chat.id,
            "🤝 You've left one of our required groups/channels.\n"
            "Please rejoin all 3 below, then tap <b>✅ Verify</b> to continue.",
...
def h_profile(m):
    ...
    if u["ref_count"] >= ref_target:
        ref_line = f"🏆 {ref_target} referrals reached! Contact admin @{md_escape(ADMIN_USERNAME)} to claim VIP ⭐"
    else:
        ref_line = f"💡 Reach {ref_target} referrals → contact admin for VIP"
    # Special rate line
    spec_line = f"✨ *Special Rate* ✅ Active — +{CURRENCY}{SPECIAL_RATE_BONUS:.2f}/OTP" if u["special_rate"] else ""
    txt = (
        "👤 *Profile*\n"
        f"━━━━━━━━━━━━━━━━━\n"
        ...
        f"✨ Special Rate: {'✅ Yes (+৳' + f'{SPECIAL_RATE_BONUS:.2f}' + '/OTP)' if u['special_rate'] else '❌ No'}\n"
...
def h_refer(m):
    ...
    if cnt >= target:
        extra = f"🏆 You've reached *{target} referrals*!\n📩 Contact admin @{md_escape(ADMIN_USERNAME)} to claim *VIP* ⭐"
    else:
...
ADMIN_BTNS = {A_NUM, A_USR, A_SET, A_FIN, A_STA, A_BC, A_SUB, A_API, A_BACK,
              ...
              U_LIST, U_BAN, U_UNBAN, U_VIPG, U_VIPR, U_SPEC, U_REF, U_BACK,
...
def adm_spec(m):
    ...
    out = [f"✨ *Special Rate Users* (+৳{SPECIAL_RATE_BONUS:.2f}/OTP)", "━━━━━━━━━━━━━━━━━━━━"]
    kb = types.InlineKeyboardMarkup(row_width=1)
    if not rows:
...
# Referrals — who referred whom (fake-referral detection)
def _profile_link(uid: int, username: str, name: str) -> str:
    """HTML link that opens the user's Telegram profile, username or not."""
    label = (name or username or str(uid)).strip()
...
@bot.message_handler(func=lambda m: m.text == U_REF and _admin_only(m))
def adm_referrals(m):
    with db() as conn:
        rows = conn.execute(
            "SELECT id, name, username, ref_count FROM users "
            "WHERE ref_count > 0 ORDER BY ref_count DESC LIMIT 50"
        ).fetchall()
    if not rows:
        return bot.send_message(m.chat.id, "👥 কেউ এখনো রেফার করেনি।", reply_markup=usr_menu_kb())
    kb = types.InlineKeyboardMarkup(row_width=1)
    for r in rows:
        label = r["username"] or r["name"] or str(r["id"])
        kb.add(types.InlineKeyboardButton(
            f"👤 {label} — {r['ref_count']} referrals",
            callback_data=f"refview|{r['id']}"
        ))
    bot.send_message(m.chat.id, "👥 *রেফারার লিস্ট* (চাপো কে কাকে রেফার করেছে দেখতে):",
                     reply_markup=kb)

@bot.callback_query_handler(func=lambda cq: cq.data.startswith("refview|"))
def cb_refview(cq):
    if not is_admin(cq.from_user.id):
        return bot.answer_callback_query(cq.id)
    try:
        ref_id = int(cq.data.split("|", 1)[1])
    except Exception:
        return bot.answer_callback_query(cq.id, "⚠️ Bad data")
    with db() as conn:
        ref_row = conn.execute("SELECT id, name, username FROM users WHERE id=?", (ref_id,)).fetchone()
        referred = conn.execute(
            "SELECT id, name, username, otp_count, joined_at, banned FROM users "
            "WHERE ref_by=? ORDER BY joined_at DESC", (ref_id,)
        ).fetchall()
    bot.answer_callback_query(cq.id)
    if not ref_row:
        return bot.send_message(cq.message.chat.id, "❌ User not found.")
    ref_label = _profile_link(ref_row["id"], ref_row["username"], ref_row["name"])
    out = [f"👥 <b>{ref_label}</b> এর রেফার করা ইউজাররা ({len(referred)}):", "━━━━━━━━━━━━━━━━━━━━"]
    if not referred:
        out.append("<i>(কেউ নেই)</i>")
    for r in referred:
        link = _profile_link(r["id"], r["username"], r["name"])
        flag = " 🚫" if r["banned"] else ""
        suspicious = " ⚠️" if not r["username"] and not r["otp_count"] else ""
        out.append(f"• {link} — OTP: {r['otp_count']} — {r['joined_at']}{flag}{suspicious}")
        ...
        if cur.rowcount == 0:
            ...
            if row:
                try: bot.send_message(row["id"],
                    f"✨ তোমাকে *Special Rate* দেওয়া হয়েছে! এখন থেকে প্রতি OTP-তে +{CURRENCY}{SPECIAL_RATE_BONUS:.2f} বেশি পাবে।"
                )
                except Exception: pass
            bot.send_message(m.chat.id, f"✅ @{uname} এখন +{CURRENCY}{SPECIAL_RATE_BONUS:.2f}/OTP পাবে।", reply_markup=usr_menu_kb())
...
def _do_specadd_id(m):
    ...
    with db() as conn:
        ...
        if cur.rowcount == 0:
            ...
            bot.send_message(m.chat.id, f"✅ ID `{tid}` এখন +{CURRENCY}{SPECIAL_RATE_BONUS:.2f}/OTP পাবে।", reply_markup=usr_menu_kb())
        ...
        try:
            ...
            for o in otps:
                ...
                if match:
                    ...
                    # One OTP per number: if this number was already credited, drop the repeat entirely
                    if already_credited_live:
                        log.info(f"live otp: skipping duplicate for {number} (already credited)")
                        continue
                    with db() as conn:
                        urow = conn.execute(
                            "SELECT special_rate FROM users WHERE id=?", (uid,)
                        ).fetchone()
                        credit = rate + (SPECIAL_RATE_BONUS if urow and urow["special_rate"] else 0.0)
                        conn.execute(
                ...
                # Mirror to OTP group (masked) — only for first OTP of a number
                try:
                    bot.send_message(
                        OTP_CHAT_ID,
...
def otp_group_handler(m):
    ...
    # One OTP per number: silently drop repeat OTPs (no group post, no DM, no credit)
    if already_credited:
        log.info(f"otp group: skipping duplicate for {matched['num']} (already credited)")
        return
    credit = 0.0
    with db() as conn:
        ...
        # Special Rate = +০.৭০ টাকা বোনাস প্রতি OTP
        if u["special_rate"]:
            bonus += SPECIAL_RATE_BONUS

        credit = rate + bonus

        # First (and only) OTP for this number — credit user and mark number
        conn.execute(
            "UPDATE users SET balance=balance+?, otp_count=otp_count+1 WHERE id=?",
            (credit, uid),
        )
